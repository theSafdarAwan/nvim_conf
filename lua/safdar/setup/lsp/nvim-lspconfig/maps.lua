...
M.on_attach = function(_, buf_nr) -- will give client and bufnr
	...
	set_map("n", "<space>wr", "<cmd>lua vim.lsp.buf.remove_workspace_folder()<CR>", opts:silent():noremap():buffer(buf_nr))
	...
	set_map("n", "<space>nf", "<cmd>lua vim.lsp.buf.format({async = true, timeout_ms = 2000 })<CR>", opts:silent():noremap():buffer(buf_nr))
